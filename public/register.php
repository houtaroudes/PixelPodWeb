...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if ($_POST['password'] !== ($_POST['password_confirm'] ?? '')) $error = 'Passwords do not match.';
    ...
    $r = registerUser($_POST, $_POST['csrf_token'] ?? '');
        ...
        <?php else: ?>
          <form method="POST">
            ...
            <div class="form-group">
              ...
              <input type="text" name="username" required placeholder="e.g. maria_santos" pattern="[a-zA-Z0-9_]{3,30}" title="3-30 characters. Letters, numbers, underscores only." value="<?= htmlspecialchars($_POST['username'] ?? '') ?>">
            </div>
            <div class="form-row">
              <div class="form-group"><label>First Name *</label><input type="text" name="first_name" required placeholder="Maria" value="<?= htmlspecialchars($_POST['first_name'] ?? '') ?>"></div>
              <div class="form-group"><label>Last Name *</label><input type="text" name="last_name" required placeholder="Santos" value="<?= htmlspecialchars($_POST['last_name'] ?? '') ?>"></div>
            </div>
            <div class="form-group"><label>Email <small style="color:var(--text-light);font-weight:400">(optional)</small></label><input type="email" name="email" placeholder="you@email.com" value="<?= htmlspecialchars($_POST['email'] ?? '') ?>"></div>
            ...
            <div class="form-row">
              <div class="form-group"><label>Password * <small style="color:var(--text-light);font-weight:400">(min 8 chars, 1 uppercase, 1 number)</small></label><input type="password" name="password" required minlength="8" placeholder="••••••••"></div>
              <div class="form-group"><label>Confirm Password *</label><input type="password" name="password_confirm" required minlength="8" placeholder="••••••••"></div>
            </div>
